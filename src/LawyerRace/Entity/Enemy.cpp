...
Enemy::Enemy(std::vector<lwe::TextureRegion*> regions,
             float x,
             float y,
             float w,
             float h,
...
void Enemy::render(SDL_Renderer* const renderer, const float timeAlpha)
{
...
void Enemy::updateMovement(const float timeStep)
{
  ...
  if (tmp_movingY && (getY() == targetYCompare))
  {
    tmp_movingX = true;
  }

  if (tmp_movingX && (getX() == targetXCompare))
  {
    tmp_movingY = true;
  }
  ...
  if (tmp_movingY)
  {
    if (getY() < targetYCompare)
    {
      ...
      if ((getY() + delta_y) + getHeight() > config.getGameHeight())
      {
        ...
        setY((float)(config.getGameHeight() - getHeight()));
        delta_y = 0.0;
      }
  ...
  if (tmp_movingX)
  {
    if (getX() < targetXCompare)
    {
      ...
      {
        setX((float)(config.getGameWidth() - getWidth()));
        delta_x = 0.0;
      }
...
void Enemy::updateTarget()
{
  ...
  //Set initial target
  for (Entity* const& e : getManager()->getAll<Player>())
  {
    Player* p = reinterpret_cast<Player*>(e);
    if (p->isMoving() && !p->isDead())
    {
